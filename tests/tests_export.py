...
def test_export_csv_other_authenticated(client, user, other_expense):
    ...
    assert data_row[3] == f"{other_expense.price:.2f}"
...
def test_export_csv_service_authenticated(client, user, service_record):
    ...
    assert data_row[5] == f"{service_record.price:.2f}"
...
def test_export_csv_fuel_authenticated(client, user, fuel_expense):
    ...
    assert data_row[4] == f"{fuel_expense.fuel_amount:.2f}"
    assert data_row[5] == f"{fuel_expense.price:.2f}"
...
def test_export_csv_carpart_authenticated(client, user, carpart):
    ...
    headers = next(csv_reader)
    assert headers == [
        "Date purchase", "Item name", "Car", "Carpart type", "Price", "Place purchase",
    ...
    assert data_row[4] == f"{carpart.price:.2f}"
